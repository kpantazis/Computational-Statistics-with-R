...
SP2GGSQK <- function(x1,x2,Increment,BandWidth,T){
  ### m=2,h(x)=(log(x),x). K means Kernel Density Estimate.
  ...
  minusloglike <- function(theta) {
    sum(log(1+rho*exp(theta[1] + theta[2]*(log(t)) + theta[3]*(t)))) -
      sum(theta[1]+theta[2]*(log(x1))+theta[3]*(x1))}
  ...
  p <- 1/(n2*(1+rho*exp(alpha + beta*(log(t)) + gamma*(t))))
  p1 <- exp(alpha + beta*(log(t)) + gamma*(t))*p
  ...
  mtext("m=2, h(x)=(log(x),x)", cex=1.2, line=1,side=3,outer=TRUE)
  
  #dev.off() #Need dev.off for png(file="gEstimate.png")     
  
  ###The LR Test gamma=0
  logL <- -sum(log(1+rho*exp(alpha + beta*(log(t)) + gamma*(t)))) +
    sum(alpha + beta*(log(x1)) + gamma*(x1)) 
  
  #MINUS log-likelihood under H_0: gamma=0
  minusloglike0 <- function(theta) {
    sum(log(1+rho*exp(theta[1] + theta[2]*log(t) + 0*(t)))) -
      sum(theta[1]+theta[2]*log(x1)+0*(x1))}
  ...
  logL0 <- -sum(log(1+rho*exp(alpha0 + beta0*log(t) + 0*(t)))) +
    sum(alpha0 + beta0*log(x1) + 0*(x1)) 
  ...
  ALPHA <- -log(sum(exp(beta*log(t) + gamma*t)*p))
...
#Note that after a few iterations of the second example, the pval_LR is larger than the pvalue threshold,but not as large 
#as we would may like. One possible way to fix this is to change the kernel density estimator to a more appropriate one,
#a density estimator for gamma distributions.